...
## Generate CSV file with input variables for ggH vs. QCD discrimination
...
def main():
    ...
    print('\nInside HtoAA_ggH_BDT_inputs3\n')

    in_file_names = []
    in_dir = '/eos/cms/store/group/phys_susy/HToaaTo4b/NanoAOD/2018/'
    if LABEL == 'JetHT':         in_dir += 'data/PNet_v1_2023_10_06/Run2018-UL2018_MiniAODv2_GT36-v1/JetHT/skims/Hto4b_0p8/'
    if LABEL == 'GluGluH':       in_dir += 'MC/PNet_v1_2023_10_06/SUSY_GluGluH_01J_HToAATo4B_Pt150_M-All_TuneCP5_13TeV_madgraph_pythia8/skims/Hto4b_0p8/'
    if LABEL == 'TTToSemiLep':   in_dir += 'MC/PNet_v1_2023_10_06/TTToSemiLeptonic_TuneCP5_13TeV-powheg-pythia8/skims/Hto4b_0p8/'
    ...
    for iEvt in range(nEntries):
        ...
        if 'TTToSemiLep' in LABEL and len(cVecs) + len(qVecs) < (CUT_GEN_BBQ + CUT_GEN_BBQQ):
        ...
        if CUT_GEN_BBQ or CUT_GEN_BBQQ:
            nQinBB = 0
            for xVec in cVecs+qVecs:
                if max(bVecs[0].DeltaR(xVec), bVecs[1].DeltaR(xVec)) < 1.6:
                    nQinBB += 1
            if nQinBB < CUT_GEN_BBQ + CUT_GEN_BBQQ: continue
        ...
        for iFat in range(ch.nFatJet):
            ...
            if fatVec.DeltaR(lepVecs[0]) > CUT_DR_LJ: continue
        ...
        for jFat in range(len(fatVecs)):
            ...
            nBinJ = (fatVec.DeltaR(bVecs[0]) < 0.8) + (fatVec.DeltaR(bVecs[1]) < 0.8)
            nQinJ = 0
            nCinJ = 0
            for xVec in qVecs:
            ...
            if CUT_GEN_BB   and nBinJ       < 2: continue
            if CUT_GEN_BBQ  and nQinJ+nCinJ < 1: continue
            if CUT_GEN_BBQQ and nQinJ+nCinJ < 2: continue
            if isTTBar:
                if (not CUT_GEN_BB) and (nBinJ == 2): continue
            nPassGen += 1
