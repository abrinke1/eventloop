...
PRT_MVA_IN = True  ## Print inputs to MVA, and use looser selection to boost training statistics
...
def main():
    ...
    if LABEL == 'TTJets_DiLept': in_dir += 'MC/TTJets/TTJets_DiLept_TuneCP5_13TeV-madgraphMLM-pythia8/Nano25Oct2019/SkimsTTBar/'
    ...
    if not LABEL.split('_')[0] in in_dir:
        ...
        sys.exit()
    if not 'TTJets' in in_dir and CUT_GEN_BB:
    ...
    hst['doubB_fat']     = R.TH1D('h_doubB_fat',     'AK8 jet btagDDBvL score', 55, -0.1, 1.0)
    ...
    for iEvt in range(nEntries):
        ...
        if iEvt % PRT_EVT is 0: print 'Looking at event #%d / %d' % (iEvt, nEntries)
        ...
        lastPdgID = -999
        genLepMoms = []
        for iGen in range(-1 if (not 'TTJets' in LABEL) else ch.nGenPart):
        ...
        for jFat in range(len(fatVecs)):
            ...
            hst['doubB_fat']    .Fill( max(-0.099, min(0.999, ch.FatJet_btagDDBvL[fatIdx]) ), WGT)
